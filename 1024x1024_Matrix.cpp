// === VGC 2.5 PPE Matrix Benchmark (Single-Core, 1024x1024 Matrix Multiply) ===
// Compile:
// g++ -O3 -march=native -std=c++17 ppe_matrix_1024.cpp -lpsapi -o matrix1024.exe
...
// =======================================================================
// Matrix Multiplication (Cache-Friendly, Single-Core)
// 1024 x 1024 workload (heavy, single-core, no partition)
// =======================================================================
...
int main() {
    const int N = 1024;  // matrix size (1024 x 1024)

    sys::pin_to_core_and_boost(0);

    std::cout << "=== VGC 2.5 PPE Matrix Benchmark (Single-Core, 1024x1024) ===\n";
    ...
    // Cache-friendly multiply: i-k-j order
    for (int i = 0; i < N; ++i) {
        for (int k = 0; k < N; ++k) {
            float aik = A[i][k];
            // inner loop accesses B[k][j] and C[i][j] sequentially in j
            for (int j = 0; j < N; ++j) {
                C[i][j] += aik * B[k][j];
            }
        }
