// === VGC 2.5 PPE Deep Recursion Benchmark (Single-Core, 80K Steps, 8K Depth) ===     //
// Compiler                                                                           //
// g++ -O3 -march=native -std=c++17 ppe_deeprecursion_80k.cpp -lpsapi -o deep80k.exe //
...
// ======================================================================= //
// Deep Recursion: depth = 8000                                           //
// Heavy stack load but safe due to chunking.                            //
// ==================================================================== // 
short deep_recurse(int depth, int limit, short acc) {
    ...
    return deep_recurse(
        depth + 1,
        limit,
        acc + static_cast<short>((depth * 9 + 5) % 32767)
    );
}

// Wrapper that repeats deep recursion in safe chunks.
short deep_driver(int total_steps, int chunk_depth = 8000) {
    short acc = 0;
    int done = 0;
...
int main() {
    const int TOTAL = 80000;     // total logical deep recursion steps
    const int DEPTH = 8000;      // deep recursion depth per chunk
